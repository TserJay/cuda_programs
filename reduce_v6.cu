...
// reduce_v5 latency = 4.144704 ms
// reduce_v5 latency = 4.035072 ms

// v5: 将所有的循环都展开


// V0-V5 中每个kernel得到了的是每个block 所负责范围的结果（归约的结果）
// 想要得到最终的结果，需要将每个block的结果进行再做reduce_sum操作，得到最终的结果。


// V6:multi-block reduce final result by two pass
// reduce_v6 latency = 8.146048 ms
...
__global__ void reduce_v6(float* d_in, float* d_out, int nums) {
	...
	unsigned int total_threads_nums = gridDim.x * blockDim.x; // 总的线程数

	float sum = 0.0f; // 存储最后的结果

	// 基于V5改进优化：不显式指定一个线程处理2个元素，通过L100的循环来自动确定每个线程处理的元素个数
...
int main() {
	...
	dim3 Block(blockSize); //每个Block的线程数
	...
	cudaMemcpy(out, d_out,	1 * sizeof(float), cudaMemcpyDeviceToHost);//将结果从设备复制到主机
